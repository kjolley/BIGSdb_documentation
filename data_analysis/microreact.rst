...
The Microreact plugin generates trees calculated from either concatenated 
sequence alignments or from allelic profiles of selected loci (or the loci 
belonging to a particular scheme). Neighbour-joining trees (generated by 
ClustalW) and Approximately-Maximum Likelihood trees (generated by FastTree)
are supported from concatenated sequences. Neighbour-joining trees using RapidNJ
(as implemented in :ref:`GrapeTree<grapetree>`) are supported for profiles.
These trees are then sent together with country and year field values to the 
`Microreact website <https://microreact.org>`_ for display.
